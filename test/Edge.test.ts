...
describe('Edge', () => {
    ...
    const createSamplePolygon = () => {
        ...
        const edges = vertices.map((v, i) => 
            new Edge(v, vertices[(i + 1) % vertices.length])
        );

        // Set up the edges for each vertex
        vertices.forEach((vertex, i) => {
            vertex.setPrevEdge(edges[(i - 1 + edges.length) % edges.length]);
            vertex.setNextEdge(edges[i]);
        });
    ...
    //             new Vector(Math.SQRT1_2, -Math.SQRT1_2),  // A
    //             new Vector(Math.SQRT1_2, Math.SQRT1_2),   // B
    //             new Vector(Math.SQRT1_2, Math.SQRT1_2),   // C
    //             new Vector(Math.SQRT1_2, Math.SQRT1_2),   // D
    //             new Vector(-Math.SQRT1_2, Math.SQRT1_2),  // E
    //             new Vector(-Math.SQRT1_2, -Math.SQRT1_2)  // F
    //         ];
    ...
    //         const { vertices } = createSamplePolygon();
    //         const reflexVertex = vertices[2]; // Vertex C
    //         const bisector = reflexVertex.calculateBisector();
            
    //         // For a reflex vertex, bisector should point inward
